...
int elegir_pagina_para_reemplazo (ssistema * S)
{
    ...
    // Búsqueda secuencial del marco ocupado cuya página
    // tenga el menor timestamp
    victima = S->tdm[0].pagina;
    for (marco = 1; marco < S->nummarcos; marco++)
    {
        if (S->tdp[S->tdm[marco].pagina].timestamp < S->tdp[victima].timestamp)
        {
...
void mostrar_informe_reemplazo (ssistema * S)
{
    ...
    // con el timestamp mínimo
    minimo = S->tdm[0].pagina;
    for (marco = 1; marco < S->nummarcos; marco++)
    {
        if (S->tdp[S->tdm[marco].pagina].timestamp < S->tdp[minimo].timestamp)
        {
    ...
    // Búsqueda secuencial de las páginas presentes en memoria
    // con el timestamp máximo
    maximo = S->tdm[S->nummarcos - 1].pagina;
    for (marco = 0; marco < S->nummarcos - 1; marco++)
    {
        if (S->tdp[S->tdm[marco].pagina].timestamp > S->tdp[maximo].timestamp)
        {
    ...
    printf ("Reemplazo LRU "
            "Reloj: %d Timestamp mínimo: %d y máximo: %d\n",
            S->reloj, S->tdp[minimo].timestamp, S->tdp[maximo].timestamp);
}
