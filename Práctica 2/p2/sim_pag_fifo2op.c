...
int elegir_pagina_para_reemplazo (ssistema * S)
{
    ...
    do
    {
        ...
        victima = S->tdm[marco].pagina; // La victima es la página del primer de la lista
        referenciada = S->tdp[victima].referenciada; // Para encontrar una página que tenga el bit de referencia a 0
        S->tdp[victima].referenciada = 0;
        S->listaocupados = marco; // Se mueve al final
    } while (referenciada == 1); // Si el bit de referencia es 1, se continua
    
    if (S->detallado)
        printf ("@ Eligiendo (FIFO con segunda oportunidad) P%d de M%d para "
                "reemplazarla\n", victima, marco);
...
void mostrar_tabla_de_paginas (ssistema * S)
{
    ...
    for (p=0; p<S->numpags; p++)
        ...
        printf ("%8d   %6d     %8s   %6s   %6d\n", p,
                S->tdp[p].presente, "-", "-", S->tdp[p].referenciada);
}
...
void mostrar_informe_reemplazo (ssistema * S)
{
    ...
    // Se muestra el bit de referencia de la página almacenada en cada marco
    for (int m = 0; m < S->nummarcos; m++)
    {
        int p = S->tdm[m].pagina;
        if (p != -1)
        {
            printf("P%d Referenciada: %d\n", p, S->tdp[p].referenciada);
        }
    }
}